---
title: "Tarea 2 - Estudio de la homogeneidad de la infección para los datos de España"
...
En esta tarea vamos a realizar un estudio de la homogeneización de la infección por comunidades autónomas dentro de España. Es decir, a partir del test $\chi^2$ vamos a ver si el virus se expandió por igual en todas las comunidades autónomas de España.
...
| fecha | tipo | Andalucía | Aragón | Asturias | Baleares | Canarias | Cantabria | ... | Navarra | País Vasco |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 2020-02-27 | casos | 1 | 0 | 0 | 1 | 6 | 0 | ... | 0 | 0 |
| 2020-02-28 | casos | 6 | 1 | 0 | 1 | 6 | 0 | ... | 0 | 0 |
| 2020-02-29 | casos | 8 | 1 | 0 | 2 | 6 | 0 | ... | 0 | 2 |
| 2020-03-01 | casos | 12 | 0 | 1 | 2 | 7 | 1 | ... | 1 | 3 |
| 2020-03-02 | casos | 12 | 0 | 1 | 2 | 7 | 10 | ... | 1 | 9 |
| 2020-03-03 | casos | 13 | 0 | 1 | 2 | 7 | 10 | ... | 1 | 13 |
...
```{r}
data <- read.csv("data/DatosCCAA.csv")
data$fecha %<>% ymd()
...
x = data$fecha
ggplot(data, aes(x)) +
  geom_line(aes(y = Andalucía, colour = "Andalucía")) +
  geom_line(aes(y = Aragón, colour = "Aragón")) +
  geom_line(aes(y = Asturias, colour = "Asturias")) +
  geom_line(aes(y = Baleares, colour = "Baleares")) +
  geom_line(aes(y = Canarias, colour = "Canarias")) +
  geom_line(aes(y = Cantabria, colour = "Cantabria")) +
  geom_line(aes(y = `Castilla-La Mancha`, colour = "Castilla-La Mancha")) +
  geom_line(aes(y = `Castilla y León`, colour = "Castilla y León")) +
  geom_line(aes(y = Cataluña, colour = "Cataluña")) +
  geom_line(aes(y = `C. Valenciana`, colour = "C. Valenciana")) +
  geom_line(aes(y = Extremadura, colour = "Extremadura")) +
  geom_line(aes(y = Galicia, colour = "Galicia")) +
  geom_line(aes(y = Madrid, colour = "Madrid")) +
  geom_line(aes(y = Murcia, colour = "Murcia")) +
  geom_line(aes(y = Navarra, colour = "Navarra")) +
  geom_line(aes(y = `País Vasco`, colour = "País Vasco")) +
...
```{r ,eval=FALSE}
# No he entendido bien como realizar el procedimiento de este ejercicio, así que he copiado y pegado
fecha = "2020-03-15"

freq_emp = data[data$fecha==fecha & data$tipo=="casos", ]
freq_emp = freq_emp[c(-1, -2)]
freq_emp_sum = sum(freq_emp)

data2 = read.csv("data/Fichero_poblacionCCAA.csv")

pob_ccaa = data2$Pob_CCAA_2019
pob_total = sum(pob_ccaa)

freq_esperada = freq_emp_sum*data2$ccaa/pob_total
```
...
```{r ,eval=FALSE}
chisq.test(freq_emp, p = freq_esperada/sum(freq_esperada))
```
...
data = data[data$tipo == "fallecidos", ]
...
freq_emp = data[data$fecha==fecha & data$tipo=="fallecidos", ]
freq_emp = freq_emp[c(-1, -2)]
freq_emp_sum = sum(freq_emp)
...
data = data[data$tipo == "hospitalizados", ]
...
freq_emp = data[data$fecha==fecha & data$tipo=="hospitalizados", ]
freq_emp = freq_emp[c(-1, -2)]
freq_emp_sum = sum(freq_emp)
