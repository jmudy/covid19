...
```{r}
tabla.Spain = data.frame(unique(covid19.Spain$Date),infectados.por.dia[,2],
                          fallecidos.por.dia[,2],recuperados.por.dia[,2])
names(tabla.Spain) = c("Fecha", "Infectados","Fallecidos", "Recuperados")
```
...
```{r}
infectados = ts(tabla.Spain$Infectados,frequency = 7,start=c(1,3))
infectados
```
...
```{r}
fallecidos = ts(tabla.Spain$Fallecidos,frequency = 7,start=c(1,3))
plot.ts(fallecidos)
```
