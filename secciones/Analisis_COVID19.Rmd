...
Sys.setenv(RETICULATE_PYTHON = "C:/Users/mudar/anaconda3/envs/covid19")
...
```{r}
my_model <- lm(log(Casos_Confirmados) ~ Dias + log(Dias) + I(Dias^2) + I(Dias^3) + sqrt(Dias),
               data = datos_spain[datos_spain$Casos_Confirmados> 0, ])
summary(my_model)
```
...
datos_por_fecha_ts = xts(x = data.frame(Real = c(datos_spain$Casos_Confirmados, rep(NA,length(pred1)- length(datos_spain$Casos_Confirmados))),
                                        ...
                                        Mod_Exp = pred2,
                                        Mod_Pot = pred3,
                                        Mod_Mixt = pred4 # Aparece una cosa sin sentido
                         ),
                         order.by = dates)
dygraph(datos_por_fecha_ts)
```
