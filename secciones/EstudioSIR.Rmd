...
reticulate::use_python("C:/Users/mudar/anaconda3/envs/covid19")
...
Vamos a cargarla:
```{r}
covid19=read.csv("../data/covid_19_clean_complete.csv")
covid19$Date=as.Date(as.character(covid19$Date),"%Y-%m-%d")
```
...
```{r}
covid19.Spain = covid19[covid19$Country.Region=="Spain",]
```

A continuación calculamos los infectados y los recuperados por días:
```{r}
infectados.por.dia = aggregate(covid19.Spain$Confirmed ~ covid19.Spain$Date,FUN=sum)
fallecidos.por.dia = aggregate(covid19.Spain$Deaths ~ covid19.Spain$Date,FUN=sum)
infectados.por.dia2 = infectados.por.dia[,2] + fallecidos.por.dia[,2]
recuperados.por.dia = aggregate(covid19.Spain$Recovered ~ covid19.Spain$Date,FUN=sum)
```
...
```
Por último creamos la tabla `tabla.Spain` con toda la información:
```{r}
tabla.Spain = data.frame(unique(covid19.Spain$Date),susceptibles.por.dia,
                          infectados.por.dia2,recuperados.por.dia[,2])
names(tabla.Spain) = c("Fecha", "Susceptibles","Infectados", "Recuperados")
```
...
```{r,eval=FALSE}
x=tabla.Spain$Recuperados
y=habitantes*log(tabla.Spain$Susceptibles)
summary(lm( y ~ x))
```
...
```{r}
dia.último = length(tabla.Spain[,1])
exp(estimación.R0*tabla.Spain$Recuperados[dia.último]/habitantes)
```
