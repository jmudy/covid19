...
Ponemos aquí distintos enlaces de los datos y estadísticas oficiales de Ministerio de Sanidad de España
...
Podéis clonar todo el repositorio (contiene muchos más datos) o solo los datos del COVID-19 desde
 
[Datadista github COVID-19](https://github.com/datadista)
...
poblacion_CCAA_wider=poblacion_CCAA_long %>%
  tidyr::pivot_wider(names_from = Periodo, values_from = Total)
...
```{r}
aux1 = ccaa_altas_long %>%
  tidyr::pivot_wider(names_from = fecha, values_from = total)
glimpse(aux1)
knitr::kable(aux1)
```
...
Los datos epidemiológicos son los que e contabilidad datos sobre la pandemia en cada unidad de medida. Como ya hemos explicado están en la versión **wider** y la **long**
...
camas_uci %>%
  rename(Públicos_UCI = Públicos, Privados_UCI = Privados, Total_UCI = Total)
...
```{r}
poblacion_CCAA = read_tsv("../data/DataINE_España/Poblacion_CCAA_2017_2019_2853.csv", locale = locale(grouping_mark = ".")) # Eliminar los puntos de millones y miles para que no lo considere un caracter
...
datos_COVID19_con_metadatos_long = datos_COVID19_long %>%
  inner_join(metadatos_CCAA %>%
               select(-CCAA))
```

## Guardamos los datos 

Guardaremos los datos como objetos y como csv
...
write_csv(datos_COVID19_long,
     file = "../data/Modelo_Datos/datos_COVID19_long.csv")
write_csv(metadatos_CCAA,
     file = "../data/Modelo_Datos/metadatos_CCAA.csv")
write_csv(datos_COVID19_con_metadatos_long,
     file = "../data/Modelo_Datos/datos_COVID19_con_metadatos_long.csv")
```
